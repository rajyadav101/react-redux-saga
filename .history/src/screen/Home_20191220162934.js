...
export default class Home extends React.Component {
    ...
    searchSanta = async member => {
        ...
        // }
        let random = "";
        const items = this.state.matchList.filter(item => item !== member);
        if (items.length > 0) {
            random = await items[Math.floor(Math.random() * items.length)];
        }
        let finalarray = items.matchList.filter(item => item !== random);
        finalarray.push(member);
        console.log("Filter items", items);
        // this.setState({ items: items });
        setTimeout(() => {
            this.setState({ loader: false, matchList: finalarray });
        }, 1000);
    };
    //
    memberList() {
