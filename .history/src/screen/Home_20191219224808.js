...
export default class Home extends React.Component {
    ...
    searchSanta = async member => {
        ...
        var array = await this.state.matchList; // make a separate copy of the array
        let random = "";
        if (array.length > 1) {
            var index = await array.indexOf(member);
            let temp = await array.splice(index, 1);
            random = await temp[Math.floor(Math.random() * temp.length)];
            let deleteSantaIndex = await temp.indexOf(random);
            let temp2 = await temp.splice(deleteSantaIndex, 1);
            await temp2.push(member);
            await this.setState({ matchList: temp, matchSanta: random });
        } else {
            random = array[0];
